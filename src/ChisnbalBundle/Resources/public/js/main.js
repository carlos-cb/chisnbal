...
//Pretty Photo
  jQuery(document).ready(function(){
    jQuery("a[data-rel^='prettyPhoto']").prettyPhoto({theme: 'dark_square'});
  });
...
//Scroll To Me
	...
	$('.scrolly a').on('click', function(e) {
	    e.preventDefault();
	    var aniLocation = jQuery(this).attr('href');
	    jQuery('body, html').animate({
	        scrollTop : jQuery(aniLocation).offset().top
	    }, {
	        duration: 1000,
	        queue: false,
	        specialEasing: {
    ...
    $('.scrollyNoStyle a').on('click', function(e) {
        e.preventDefault();
        var aniLocation = jQuery(this).attr('href');
        jQuery('body, html').animate({
            scrollTop : jQuery(aniLocation).offset().top
...
/*
    jQuery('div.specs').bind('inview', function (event, visible) {
      if (visible == true) {
		setTimeout(
		  function() 
		  {
		    jQuery(".spec1").css("opacity", "1"); 
		    jQuery(".line1").css("opacity", "1"); 
		  }, 1500);
        
      } else {
        jQuery(".spec1").css("opacity", "0"); 
        jQuery(".line1").css("opacity", "0"); 
      }
    });*/
...
jQuery(window).load(function() { jQuery("#spinner").delay( 2000 ).fadeOut("slow"); })
 
//Div window height
jQuery(function(){
    jQuery(window).load(function(){ // On load
        jQuery('.supersizedBackground').css({'height':((jQuery(window).height()) - 72 )+'px'});
    });
